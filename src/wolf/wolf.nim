...
proc main(): int =
    ...
    if paramRepl.found:
        ...
        if paramBytecode.found:
            ...
            var slen: csize = 0
            var serialized: ptr uint8 = wolf_bytecode_serialize(addr(instance.bytecode), addr(slen), paramDebug.found)

            if paramDebug.found:
                noticeln("Serlaized bytecode: ")
                for i in 0..slen:
                    {.emit: "wolf_print_raw(\"%02hhx \", serialized[i]);".}
                printRaw("\n")
